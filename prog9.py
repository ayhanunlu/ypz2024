...
def hastalistele():
    def listeduzelt():
        ...
        while say<toplamkayit:
            idd=kutt[index][0]
            print (idd)
            i+=1
            kut.append(entries[i].get())
            say2+=1
            i+=1
            kut.append(entries[i].get())
            say2+=1
    ...
    for k in range(toplamkayit):
        for i in range(3):
            ...
            entries.append(en)
    m=-1
    for j in range(toplamkayit):
        m+=1
        entries[m].insert(0,kutt[j][1])
        m+=1
        entries[m].insert(0,kutt[j][2])
        m+=1
        entries[m].insert(0,kutt[j][3])
    ...
    """while kont<toplamkayit-1:
        #bol=kutt[kont].split(",")
        index+=1
        entries[index].insert(0,kutt[index][1])
        index+=1
        entries[index].insert(0,kutt[index][2])
        index+=1
        entries[index].insert(0,kutt[index][3])
        kont+=1"""
